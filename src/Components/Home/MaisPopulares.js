...
function MaisPopulares() {
  ...
  const classNames = "hover:bg-dry transitions text-sm rounded w-8 h-8 flex-colo bg-dry text-white"
  return (
    <div className='my-16'>
        ...
        <Swiper navigation={{ nextEl, prevEl }} slidesPerView={4} spaceBetween={40} autoplay={true} speed={1000} loop={true} modules={[Navigation, Autoplay]}>
          ...
          {Movies.map((movie, index) => (
            <SwiperSlide key={index}>
              <div className='p-4 h-rate hovered border border-border bg-border rounded-lg overflow-hidden'>
                <img src={require(`../../assets/${movie.image}`)} alt={movie.name} className="w-full h-full object-cover rounded-lg" />
